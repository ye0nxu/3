...
"""
    ...
    def load_video_from_path(
        ...
        """영상 입력 페이지에서 선택된 영상 경로·ROI를 직접 주입해 로드합니다.

        이미 동일 경로가 로드돼 있으면 ROI만 갱신하고 재로드하지 않습니다.
        실행 중이거나 내보내기 중에는 무시합니다.
        """
        ...
        if ok:
            self._append_log(f"[INFO] 영상 입력 페이지에서 자동 로드: {Path(path).name}")
            self._update_button_state()
        return ok
...
QPushButton#btnPromptSend{{background:{c["accent"]};border:1px solid {c["accent"]};border-radius:12px;color:{c["accent_text"]};font:800 10pt "Pretendard";}}
QPushButton#btnPromptSend:hover{{background:{c["accent_hover"]};border-color:{c["accent_hover"]};}}
QPushButton#btnPromptSend:disabled{{background:{c["accent_disabled"]};border-color:{c["accent_disabled"]};}}
...
"""
    ...
    def _set_video_frame(self, frame: Any, frame_index: int) -> None:
        ...
        self._current_video_frame = frame.copy() if hasattr(frame, "copy") else frame
